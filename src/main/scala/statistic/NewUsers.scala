            ...
            UserGroupInformation.getLoginUser.doAs( new PrivilegedExceptionAction[Unit] {
              def run: Unit = {
               ...
               // val hiveWareHouse = "/user/hive/warehouse"
                ...
                //执行查询方法""
                val originalData = hiveContext.sql(" select COUNT(*) from dwd_cst_consumer  where BUILDDATE >= '2020-02-26 00:00:00' AND BUILDDATE < '2020-03-26 00:00:00' AND (SGSTATUS <> '9' OR (SGSTATUS = '9' AND CANCELDATE >= '2020-03-26 00:00:00'))");

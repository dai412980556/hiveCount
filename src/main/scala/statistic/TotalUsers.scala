...
/**
  ...
  */
object TotalUsers {
  val logger: Logger = LoggerFactory.getLogger(TotalUsers.getClass)
    /** 命令行参数 */
    ...
    def run(p: Params): Unit = {
        ...
        if (CommonUtil.isEmpty(p.principal) || CommonUtil.isEmpty(p.keytab)) {
            ...
            //加载原始数据
            val originalData = SparkMLUtils
              .loadDataset(dataSourceConfig.toLocalIterator.mkString, null, p.principal, p.keytab)
            //保存结果
            ...
            UserGroupInformation.getLoginUser.doAs( new PrivilegedExceptionAction[Unit] {
              def run: Unit = {
                ...
                //执行查询方法
                val originalData = hiveContext.sql(" select COUNT(*) from dwd_cst_consumer  where BUILDDATE < '2020-03-26 00:00:00' AND (SGSTATUS <> '9' OR (SGSTATUS = '9' AND CANCELDATE >= '2020-03-26 00:00:00')) ");
